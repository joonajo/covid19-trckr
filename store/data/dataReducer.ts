...
const setData = (state: TDataState, action: TActions): TDataState => {
  if (action.type === actionTypes.SET_DATA) {
    const countries: string[] = Object.keys(action.payload.raw).map(
      (key) => key
    );
    return {
      ...state,
...
const toggleCountry = (state: TDataState, action: TActions): TDataState => {
  if (
    action.type === actionTypes.TOGGLE_COUNTRY_SELECTION &&
    state.editedData
  ) {
...
const getTotals = (state: TDataState, action: TActions): TDataState => {
  if (action.type === actionTypes.SET_TOTALS_ALL) {
  }
  return { ...state };
};
...
export const dataReducer = (
  state: TDataState = defaultDataState,
  action: TActions
): TDataState => {
  switch (action.type) {
